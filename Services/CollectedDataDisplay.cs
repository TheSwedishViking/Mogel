    ...
    internal class CollectedDataDisplay
    {
        ...
        public static async Task DisplayDailyTemp(List<WeatherData> data)
        {
            ...
            foreach (var day in recentTemp)
            {
                ...
                await Writer.WriteRow("dailytemp.txt", $"{day.Date:yy-MM-dd} | {day.Count,-8} | {day.Temp.ToString("0.0"),-8} | {day.Humidity.ToString("0.0")} | {recentMatch.Count,-8} | {recentMatch.Temp.ToString("0.0"),-7} | {recentMatch.Humidity.ToString("0.0")}");
            }
